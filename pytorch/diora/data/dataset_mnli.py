...
class ConsolidateDatasets(object):
    ...
    def run(self):
        ...
        for dset, inverse_mapping in zip(self.datasets, inverse_mapping_lst):
            dset['sentences_1'] = self.reindex(dset['sentences_1'], inverse_mapping)
            dset['sentences_2'] = self.reindex(dset['sentences_2'], inverse_mapping)
...
class ReaderManager(object):
    ...
    def run(self, options, text_path, embeddings_path):
        ...
        #print(reader_result['extra'].keys())
        #exit()
        sentences_1 = reader_result['sentences_1']
        sentences_2 = reader_result['sentences_2']
        extra = reader_result['extra']
        metadata = reader_result.get('metadata', {})
        logger.info('len(sentences)={}'.format(len(sentences_1)))
        #print(len(sentences_1),len(sentences_2))
        word2idx = build_text_vocab(sentences_1+sentences_2) 
        ...
        sentences_1 = indexify(sentences_1, word2idx, unk_index)
        sentences_2 = indexify(sentences_2, word2idx, unk_index)
        return {
            ...
            "extra": extra,
            "metadata": metadata,
            "example_ids":reader_result['example_ids']
        }


class ReconstructDataset(object):

    def initialize(self, options, text_path=None, embeddings_path=None, filter_length=0, data_type='nli'):
        print(data_type)
        if data_type == 'nli':
...
def make_batch_iterator(options, dset, shuffle=True, include_partial=False, filter_length=0,
    ...
    sentences_1 = dset['sentence1']
    sentences_2 = dset['sentence2']
    ...
    if options.reconstruct_mode in ('margin', 'softmax'):
        freq_dist = calculate_freq_dist(sentences_1, vocab_size)
    ...
    batch_iterator = BatchIterator(
        sentences_1,sentences_2, extra=extra, shuffle=shuffle, include_partial=include_partial,
